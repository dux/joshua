...
#
# Used by Joshua::BaseInstance#send_file and #send_data. Sets all the
...
class Joshua
  class FileResponse
    ...
    # @param api  Joshua::BaseInstance#@api struct
    # @param opts Hash:
    ...
    def resolve_content
      if @opt.content
        ...
        raise Joshua::Error, 'send_file: :file or :content required' unless @opt.file
        raise Joshua::Error, 'send_file: file not found: %s' % @opt.path unless @opt.file.exist?
        @opt.file.binread
      end
    end
