...
def sys_call(action)
  ...
  raw  = Joshua::SysApi.render(action, api_host: host)
  JSON.parse(raw, symbolize_names: true)
end

describe Joshua::Introspect do
  ...
  let(:doc) { Joshua::Introspect.schema(mount_on: '/api') }
...
describe 'Joshua::SysApi endpoints' do
  ...
  it 'web (no file) returns index.html shell' do
    ...
    html = Joshua::SysApi.render(:web, api_host: host)
  ...
  it 'web ?file=/fez/joshua-method.fez returns raw component' do
    host = SysMockApiHost.new(path: '/api/sys/web', method: 'GET')
    body = Joshua::SysApi.render(:web, api_host: host, params: { file: '/fez/joshua-method.fez' })
  ...
  it 'web ?file=boot.js returns the JS file' do
    host = SysMockApiHost.new(path: '/api/sys/web', method: 'GET')
    body = Joshua::SysApi.render(:web, api_host: host, params: { file: 'boot.js' })
  ...
  it 'web treats leading / as web-root, does not escape filesystem' do
    ...
    host = SysMockApiHost.new(path: '/api/sys/web', method: 'GET')
    result = Joshua::SysApi.new(:web, params: { file: '/etc/passwd' }, api_host: host).execute_call
  ...
  it 'web rejects scheme/remote URLs' do
    host = SysMockApiHost.new(path: '/api/sys/web', method: 'GET')
    result = Joshua::SysApi.new(:web, params: { file: 'https://evil.com/x.js' }, api_host: host).execute_call
    expect(result[:success]).to eq(false)
  ...
  it 'web rejects path traversal' do
    host = SysMockApiHost.new(path: '/api/sys/web', method: 'GET')
    result = Joshua::SysApi.new(:web, params: { file: '../joshua/sys_api.rb' }, api_host: host).execute_call
    expect(result[:success]).to eq(false)
  ...
  it 'web rejects disallowed extensions' do
    host = SysMockApiHost.new(path: '/api/sys/web', method: 'GET')
    result = Joshua::SysApi.new(:web, params: { file: 'boot.rb' }, api_host: host).execute_call
    expect(result[:success]).to eq(false)
    expect(result[:error][:messages].first).to include('extension not allowed')
  end
end

describe 'Joshua rack call' do
  ...
  it 'redirects the mount root GET to /<mount>/sys/web' do
    ...
    status, headers, _ = Joshua.call(rack_env(path: '/api'))
  ...
  it 'serves sys/schema with Content-Type application/json (CT bug regression)' do
    ...
    status, headers, body = Joshua.call(rack_env(path: '/api/sys/schema'))
